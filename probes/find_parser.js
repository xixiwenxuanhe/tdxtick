...
Interceptor.attach(k32.findExportByName('GetQueuedCompletionStatus'),{onEnter(args){this.ovp=args[3];},onLeave(ret){try{
  ...
  if(ln===0||ln>60000) return;
  const pa=buf.add(16);
  last={lo:pa, hi:pa.add(ln+64), cmd, t:Date.now()};
}catch(e){}}});
function hookMem(name){ const a=crt.findExportByName(name); if(!a)return; Interceptor.attach(a,{onEnter(args){try{
  if(!last || Date.now()-last.t>2000) return;
  const src=args[1];
  if(src.compare(last.lo)>=0 && src.compare(last.hi)<0){
    const key=this.returnAddress.toString();
    ...
    send({kind:'read',cmd:last.cmd,src:src.toString(),ret:modof(this.returnAddress),bt});
  }
}catch(e){}}}); send({kind:'hooked',name}); }
['memcpy','memmove'].forEach(hookMem);
send({kind:'ready'});
